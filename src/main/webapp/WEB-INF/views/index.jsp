...
<body class="ng-cloak">
		...
		<!-- 가장윗줄 -->
		<nav class="navbar navbar-dark bg-inverse navbar-fixed-top d-flex justify-content-between" style="color:white;width:100%;height:35px;z-index:3;background-color:#484848;padding:0 30px;position: fixed;"> <i class="xi-box" style="color: white; font-size: small" ng-if="systemk == true"> SYSTEMK 재고관리</i> <i class="xi-box" style="color: white; font-size: small" ng-if="systemk != true"> THEFACTOR2 재고관리</i> <!-- Top Menu Items -->
		<ul class="navbar-nav flex-row ml-md-auto d-md-flex " ng-if="authenticated">
				<li class="nav-item"><a class="nav-link" style="margin-right: 20px;"> {{userId}}님 반갑습니다</a></li>
				<li class="nav-item"><a class="nav-link" href="#" ng-click="logout()"><i class="xi-log-out" aria-hidden="true"></i> 로그아웃</a></li>
		</ul>
		</nav>
		...
		<div class="p-2 d-flex">
				...
				<div class="dropdown-content" ng-style="dropDown">
						<div class="dropdown-board">
								<span>시스템관리</span> <a href="" ng-repeat="value in topMenu" ng-click="goMenu(value)" ng-if="value.GROUP_CD == 'SYSTEM'"> &#176; {{value.PGM_NM}}</a>
						</div>

						<div class="dropdown-board">
								<span>기초정보관리</span> <a href="" ng-repeat="value in topMenu" ng-click="goMenu(value)" ng-if="value.GROUP_CD == 'BASE'"> &#176; {{value.PGM_NM}}</a>
						</div>

						<div class="dropdown-board">
								<span>입출고조회</span> <a href="" ng-repeat="value in topMenu" ng-click="goMenu(value)" ng-if="value.GROUP_CD == 'INOUT'"> &#176; {{value.PGM_NM}}</a>
						</div>

						<div class="dropdown-board">
								<span>재고실사</span> <a href="" ng-repeat="value in topMenu" ng-click="goMenu(value)" ng-if="value.GROUP_CD == 'INVEN'"> &#176; {{value.PGM_NM}}</a>
						</div>

						<div class="dropdown-board">
								<span>재고현황</span> <a href="" ng-repeat="value in topMenu" ng-click="goMenu(value)" ng-if="value.GROUP_CD=='STOCK'"> &#176; {{value.PGM_NM}}</a>
						...
						<div class="dropdown-board" ng-if="hiddenFunction==true">
								<span>TEST</span> <a href="/#/ex/stk"> &#176; 재고</a>
						</div>
				</div>
		</div>

		<div class="ml-auto p-2" id="menuToggle" ng-show="authenticated">
				<button class="btn" ng-click="dropDown.display = dropDown.display == 'block' ? '' : 'block'" style="background: white;">
						<i class="xi-list"></i>
				</button>
		</div>
		</nav>
		...
		<div style="height: 480px;" ng-if="authenticated && mainPage">
				<div style="overflow: hidden; background: white; width: 100%;">
						<img ng-src="{{imgPath}}/img/main.png" style="width: 100%; min-width: 1680px; display: block; margin: 0px auto;" draggable="false">
				</div>
		</div>
		<!-- regPage :사용자 관리 , 공통코드 관리, 브랜드 관리, 디바이스 관리 -->
		<!-- 상품관리 페이지 (!regPage)-->
								...
								<div class="left-body">
										...
										<div class="d-flex justify-content-between left-body-menu">
												...
												<select class="form-control" ng-model="quickSearch.storeCd" style="width: 180px;">
														...
														<option ng-repeat="value in store" value="{{value.commCd}}">{{value.commCdNm}}</option><!-- 논현본점, 압구정점 -->
												</select>
										</div>
										...
										<div class="d-flex justify-content-between left-body-menu" ng-click="st3 = st3==true ? false : true">
												...
												<i style="margin: 8px;" class="xi-angle-down" ng-show="!st3"></i> <i style="margin: 8px;" class="xi-angle-up" ng-show="st3"></i>
										</div>
						...
						<div style="width: 80%; padding: 10px;">
								...
								<div class="container-fluid d-flex justify-content-center" ng-if="!mainPage" style="padding: 0 !important;">
										<div class="d-flex align-items-end" style="width: 30%; border-bottom: 1px solid lightgray;">
												<h4>
														<!--PGM_NM:프로그램명  -->
														<a class="menuName" href="" ng-click="reload()">{{currentMenu.PGM_NM}}</a>
												</h4>
										</div>
								...
								<div class="d-flex container-fluid body-custom flex-column" style="width: 100%; padding: 5px 0 5px 0;">
										<div class="d-flex" style="border-bottom: 1px solid lightgray; margin-bottom: 4px; background: #FAFAFA; padding-bottom: 3px; padding-left: 2%;">
												...
												<!-- quickCmd.cmd :의 초기값 IO1 , PGM_NM의 value -->
										...
										<!-- 검색 1 --><!-- ['IO1','IO2','IO3'];   입고조회, 출고조회, 판매/베송조회-->
										<div class="d-flex" style="width: 100%; padding-left: 2%" ng-if="quick1">
												...
												<div class="row input-group" style="width: 200px; margin: 5px;">
														<input type="text" class="form-control" uib-datepicker-popup="{{format}}" ng-model="quickSearch.startDate" is-open="st2_sdt" datepicker-options="startDateOptions" close-text="Close" ng-readonly="dateUse" /> <span class="input-group-append">
																<button type="button" class="btn btn-secondary" ng-click="st2_sdt = st2_sdt==true ? false : true">
												...
												<div class="row input-group" style="width: 200px; margin: 5px;">
														<input type="text" class="form-control" uib-datepicker-popup="{{format}}" ng-model="quickSearch.endDate" is-open="st2_edt" datepicker-options="endDateOptions" close-text="Close" ng-readonly="dateUse" /> <span class="input-group-append">
																<button type="button" class="btn btn-secondary" ng-click="st2_edt = st2_edt==true ? false : true">
																		<i class="xi-calendar"></i>
																</button>
														</span>
												</div>
										...
										<!-- 검색 2 --><!--  ['IO4','IV1']; 입출고내역조회, 재고실사관리-->
										<div class="d-flex" style="width: 100%; padding-left: 2%" ng-if="quick2">
												<p style="margin: 12px 10px;">일자검색</p>
												<div class="row input-group" style="width: 200px; margin: 5px;">
														<input type="text" class="form-control" uib-datepicker-popup="{{format}}" ng-model="quickSearch.startDate" is-open="st2_sdt" datepicker-options="startDateOptions" close-text="Close" ng-readonly="dateUse" /> <span class="input-group-append">
																<button type="button" class="btn btn-secondary" ng-click="st2_sdt = st2_sdt==true ? false : true" style="height: 38px;">
																		<i class="xi-calendar"></i>
																</button>
														</span>
												</div>
												...
												<div class="row input-group" style="width: 200px; margin: 5px;">
														<input type="text" class="form-control" uib-datepicker-popup="{{format}}" ng-model="quickSearch.endDate" is-open="st2_edt" datepicker-options="endDateOptions" close-text="Close" ng-readonly="dateUse" /> <span class="input-group-append">
																<button type="button" class="btn btn-secondary" ng-click="st2_edt = st2_edt==true ? false : true" style="height: 38px;">
																		<i class="xi-calendar"></i>
																</button>
														</span>
												</div>
										...
										<!-- 검색 3 --><!-- ['IO1','IO2','IO3','ST1','BS3','ST2','IV2'];입고조회, 출고조회, 판매/배송조회, 재고현황관리, 상품관리, 재고상품(태그)관리, 재고실사내역조회-->
										<div class="d-flex" style="width: 100%; padding-left: 2%" ng-if="quick3">
												<input class="form-control" id="brandSearch" style="width: 220px; margin: 10px; height: 40px;" placeholder="브랜드" ng-model="view.brand" ng-click="qs1 = qs1==true ? false : true" readonly> <label class="d-flex justify-content-between" for="brandSearch" style="position: relative; left: -35px; top: 15px; width: 0px;"> <i style="margin: 10px; font-size: 11px; font-weight: bolder;" class="xi-angle-down" ng-show="!qs1"></i> <i style="margin: 10px; font-size: 11px; font-weight: bolder;" class="xi-angle-up" ng-show="qs1"></i>
												</label>
		...
		<!-- 등록 페이지 --><!-- regPage: comm, brand, user,device -->
