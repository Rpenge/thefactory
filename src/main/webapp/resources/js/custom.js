function userLoginCheck($http, $rootScope,$window, $location){
  ...
  //메뉴체크가 실행시 메뉴체크가 주소권한을 얻어올시에 메뉴 체크를 실행하라
  function menuCheck($rootScope, $location){
    var menuCehck = false ;
    var url = $location.url().split('?');
    
    //PGM_URL: 프로그램 주소 권한이 있으면 menucheck시작
  ...
  function httpGetList(http, scope, url, param ){
    ...
    http.get(url + param).success(
      function(data) {
        ...
        scope.paging.begin = parseInt((scope.paging.current-1)/10) * 10 + 1  // 1 or
                                        // -4 :
                                        // 1
                                        // 1~10,
                                        // 11~20
  ...
  function httpGetSubList(http, scope, url, param ){
    ...
    http.get(url + param).success(
      function(data) {
        ...
        scope.subPaging.begin = parseInt((scope.subPaging.current-1)/10) * 10 + 1  // 1 or
                                            // -4 :
                                            // 1
                                            // 1~10,
                                            // 11~20
